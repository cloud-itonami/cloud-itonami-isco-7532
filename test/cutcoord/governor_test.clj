(ns cutcoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [cutcoord.store :as store]
            [cutcoord.advisor :as advisor]
            [cutcoord.governor :as governor]))
...
(def ^:private req {:cutter-id "cutter-1"})
...
(deftest hard-on-unregistered-cutter
  (let [st (fresh-store)
        v (governor/check {:cutter-id "nobody"} {} (op :log-work-record) st)]
    (is (:hard? v))
    (is (some #(= :no-cutter (:rule %)) (:violations v)))))
...
(deftest hard-on-scope-excluded-op-finalize-pattern-cutting-execution-decision
  (testing "finalizing a pattern-cutting-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-pattern-cutting-execution-decision) :confidence 0.99) st)]
      (is (:hard? v))
...
(deftest hard-on-scope-excluded-op-declare-workshop-safety-clearance
  (testing "declaring the workshop safety-cleared is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :declare-workshop-safety-clearance) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to finalize the pattern-cutting execution is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we finalize the pattern-cutting execution decision now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a cutting-tool-hazard/material-handling/equipment-condition concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'cutter'/'pattern'/'fabric'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:cutter-id "cutter-1" :op o :workshop-id "W-1"
                        :stake :low :task "routine garment cutting task"
