(ns cutcoord.actor
  "CutCoordActor — the ISCO-08 7532 garment cutting workshop
  ...
  resume after human sign-off. ISCO-08 7532 (Garment and Related
  Patternmakers and Cutters) operates rotary cutters, cutting
  machines and shears on fabric — a standard workshop cutting-tool
  hazard — so this actor's scope stays deliberately limited to
  workshop scheduling/logistics coordination, never pattern-cutting
  execution or safety-clearance authority. Modeled closely on
  cloud-itonami-isco-7319's craftcoord.actor (closest published,
  ...
  The unconditional invariant: the Cut Coordination Advisor can never
  directly commit an operating record the CutCoordGovernor refuses,
  and can never make this actor finalize a pattern-cutting-execution
  decision (e.g. deciding a cut is complete/final) or a
  workshop-safety-clearance decision (e.g. declaring the workshop
  safety-cleared), nor override a shop safety officer's judgment —
  ...
  path. This actor coordinates WORKSHOP SCHEDULING/LOGISTICS ONLY —
  it never performs pattern-cutting work or makes safety-clearance
  decisions itself."
...
(defn build-graph
  "Build a compiled CutCoordActor graph. `store` implements
  `cutcoord.store/Store`. `advisor` implements
  `cutcoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:cutter-id (:cutter-id request)
                                    :op (:op proposal)
